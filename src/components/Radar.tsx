...
export const Radar = ({ className }: any) => {
  const circles = new Array(20).fill(1);
  return (
    <div
      className={twMerge(
...
export const Circle = ({ className, children, idx, ...rest }: any) => {
  return (
    <motion.div
      ...
      className={twMerge(
        "absolute inset-0 left-1/2 top-1/2 h-10 w-10  -translate-x-1/2 -translate-y-1/2 transform rounded-full border border-red-200",
        className
      )}
    ></motion.div>
  );
